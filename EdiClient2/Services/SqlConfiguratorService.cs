...
WHERE 1 = 1
  ...
  AND DJ.DOC_DATETIME {DatesBetween(date1, date2)}
        ...
        internal static string Sql_SelectGoods(string searchByNameString)
            => $@"  SELECT rg.id ""Id""
        , rg.code ""Code""
        , rg.name ""Name""
        , rg.bar_code ""BarCode""
        ...
        FROM abt.ref_goods rg
            WHERE name like '%{searchByNameString}%'";


        internal static string Sql_SelectFailedGoods(string searchByNameString)
    ...
    WHERE EDD.FAILED = 1 and upper(edd.ITEM_DESCRIPTION) LIKE upper('%{searchByNameString}%')";
